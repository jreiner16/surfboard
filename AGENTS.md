...
- `surfboard_evaluate` — Execute JavaScript on the page. Pass `js` string.
...
- `surfboard_fill_and_submit` — Type text into an input and press Enter. Pass `id`, `text`.
...
- `surfboard_highlight` — Highlight elements by their IDs (yellow outline). Pass `ids` array.

### Workflow
1. `surfboard_search` to find something
2. `surfboard_click <id>` to open a result
...
5. Use `surfboard_evaluate` with `js` to run custom JS and inspect state
6. Repeat as needed
